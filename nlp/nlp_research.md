...
- 以下の画像は`query`が`making`だった場合のAttention状況を示している．上がQueryで下がValue．`making`に対して`more`や`difficult`などに強いAttentionがあたっており、`making ... more difficult`という長距離で関係を持つ句関係を捉えていることがわかる．
